        ...
        try:
            ...
            await xx.edit(f"Total users : {x}", 
                             buttons=[
                                 [Button.inline(f"SENT: {len(sent)}", data="none")],
                                 [Button.inline(f"FAILED: {len(failed)}", data="none")]])
    await xx.edit(f"**Bʀᴏᴀᴅᴄᴀsᴛ Cᴏᴍᴘʟᴇᴛᴇ**\n\n**Tᴏᴛᴀʟ ᴜsᴇʀs ɪɴ ᴅᴀᴛᴀʙᴀsᴇ**: {x}", 
                 buttons=[
                     [Button.inline(f"SENT: {len(sent)}", data="none")],
                     [Button.inline(f"FAILED: {len(failed)}", data="none")]])
...
async def bban(event):
    ...
    if c in admins:
        return await event.reply("I Cᴀɴɴᴏᴛ Bᴀɴ Aɴ AUTH_USER")
    xx = await db.is_banned(int(c))
    if xx is True:
        return await event.reply("Usᴇʀ ɪs Aʟʀᴇᴀᴅʏ Aʟʟᴏᴡᴇᴅ!")
    else:
        await db.banning(int(c))
        await event.reply(f"{c} ɪs ɴᴏᴡ ᴅɪsᴀʟʟᴏᴡᴇᴅ")
...
async def unbban(event):
    ...
    await event.reply(f"{xx} Aʟʟᴏᴡᴇᴅ! ")
